...
Pass the Jira (or tracker) issue key and any context your runner supports: plain text after this prompt, a template variable (e.g. `$ARGUMENTS` in some agents), or an `@`-reference to a file. Whatever you receive here is the ticket identifier and optional notes.
...
1. Adopt the backend developer persona and constraints from this repository’s agent spec. Prefer `ai-specs/.agents/backend-developer.md` when it exists; otherwise use the equivalent definition your environment already loads (for example `.opencode/agents/`, Cursor rules, or `AGENTS.md` excerpts that define backend work).
2. Obtain ticket/issue content using whatever fits the run: tracker UI or API, pasted description, exported markdown, an MCP integration if available, or by reading a **local file path** the user gave. If the ticket body is already in a local file, read that file and do not depend on MCP for the same text.
3. Propose a step-by-step plan for the backend part, taking into account everything mentioned in the ticket and applying the project’s best practices and rules you can find in `docs/`.
4. Apply the best practices of your role so a developer can implement the ticket end-to-end using only your plan.
5. Do not write code yet; provide only the plan in the output format defined below.
6. If you are asked to start implementing later: first ensure work happens on a branch named after the ticket id (create or switch as needed), then follow this repo’s documented workflow (`AGENTS.md`, `docs/`, and any `ai-specs/commands/` prompts your team uses for backend or user-story development)—not a specific IDE slash command.
