...
> **Note — Current State vs. Target State**
> This document describes both what is **currently implemented** and the **target architecture** the project is evolving toward. Sections marked `[TODO]` describe patterns that are not yet in the codebase and must be built. Sections marked `[CURRENT STATE - NEEDS REFACTOR]` describe working code that exists but deviates from the target architecture and must be refactored. Do not assume these patterns are already implemented when generating code — always check whether the file or pattern exists first.
...
- **Prisma 5.13.0**: Modern ORM for database access

### Database & ORM
- **PostgreSQL**: Relational database (Docker container via `docker-compose.yml` at project root)
- **Prisma Client**: Type-safe database client
- **Prisma Migrate**: Database migration tool

### Testing Framework
- **Jest 29.7.0**: Testing framework with TypeScript support via `ts-jest`
- **Coverage Threshold (target)**: 90% for branches, functions, lines, and statements — `[TODO]` not yet configured in `jest.config.js`
- **Test Location**: `.test.ts` files alongside source files — `[TODO]` no tests exist yet

### Development Tools
- **ESLint 9.2.0**: Code linting with `eslint-config-prettier`
...
The backend targets a layered DDD architecture:
...
- Repository interfaces define data access contracts — `[TODO]` not yet created
- Pure business logic without external dependencies — `[CURRENT STATE - NEEDS REFACTOR]` models currently instantiate `PrismaClient` directly

**Infrastructure Layer** — `[TODO]` not yet created
- Prisma ORM handles database operations
- Repository implementations (via Prisma) satisfy domain interfaces
- Centralized `prismaClient.ts` singleton
- Logging utilities

### Current Project Structure

This is the **actual structure as it exists today**. Use this as the source of truth when navigating or generating code.

```
backend/
├── src/
│   ├── domain/
│   │   └── models/              # Domain entity classes (Candidate, Education, WorkExperience,
│   │                            #   Resume, Application, Interview, InterviewFlow,
│   │                            #   InterviewStep, InterviewType, Position, Company, Employee)
│   ├── application/
│   │   ├── services/
│   │   │   ├── candidateService.ts   # addCandidate, findCandidateById
│   │   │   └── fileUploadService.ts  # multer-based file upload handler
│   │   └── validator.ts              # Input validation functions
│   ├── presentation/
│   │   └── controllers/
│   │       └── candidateController.ts  # addCandidateController, getCandidateById
│   ├── routes/
│   │   └── candidateRoutes.ts    # POST /candidates, GET /candidates/:id
│   └── index.ts                  # Express app entry point (port 3010)
├── prisma/
│   ├── schema.prisma             # Database schema (single source of truth)
│   ├── seed.ts                   # Database seeding script
│   └── migrations/               # Migration history
├── api-spec.yaml                 # OpenAPI 3.0 specification (source of truth for API contracts)
├── jest.config.js                # Jest config (ts-jest preset, no coverage threshold yet)
├── tsconfig.json                 # TypeScript config (strict mode enabled)
├── .eslintrc.js                  # ESLint config (prettier/recommended)
...
This is the **desired future state** once refactoring and feature work is complete. `[TODO]` items do not exist yet.
...
│   │   └── repositories/        # [TODO] Repository interfaces (ICandidateRepository, etc.)
│   ├── application/
...
│   │   ├── logger.ts            # [TODO] Centralized logging utility
│   │   └── prismaClient.ts      # [TODO] Shared PrismaClient singleton
│   ├── routes/                  # Express route definitions
│   ├── middleware/              # [TODO] Express middleware directory
...
The following issues exist in the current codebase and must be addressed as part of ongoing development. When generating new code, do **not** replicate these patterns — follow the target architecture described elsewhere in this document.

### 1. PrismaClient instantiated in every domain model `[NEEDS REFACTOR]`
Every model file (`Candidate.ts`, `Education.ts`, `WorkExperience.ts`, `Resume.ts`, `Position.ts`, `Application.ts`, `Interview.ts`, `InterviewFlow.ts`, `InterviewStep.ts`, `InterviewType.ts`) calls `const prisma = new PrismaClient()` at the module level. This violates DIP, creates multiple database connections, and makes unit testing impossible without complex module mocking.
**Target**: Create `src/infrastructure/prismaClient.ts` as a shared singleton and inject it through repository classes.

> **Dead code warning**: `src/index.ts` already contains a Prisma injection middleware that attaches a `PrismaClient` instance to `req.prisma`, but **no controller or route currently uses it** — all existing handlers still reach through to the model-level singletons. Do **not** build new features on the `req.prisma` pattern. New code must use constructor-injected repositories as described in the Target architecture.

### 2. No repository abstraction layer `[TODO]`
Domain models contain `save()` and `static findOne()` methods that access Prisma directly. There are no repository interfaces in `src/domain/repositories/` and no repository implementations in `src/infrastructure/`.
**Target**: Define `ICandidateRepository` (and equivalents for other entities) in `src/domain/repositories/`, implement in `src/infrastructure/`.

### 3. No infrastructure layer `[TODO]`
The directories `src/infrastructure/`, `src/middleware/` do not exist. There is no centralized logger and no shared Prisma singleton.

### 4. No tests exist `[TODO]`
There are zero `.test.ts` files in the project. `jest.config.js` has no `coverageThreshold` configured.
**Target**: 90% coverage on branches, functions, lines, and statements. Add `test:coverage` script to `package.json`.

### 5. Spanish language in source code `[NEEDS REFACTOR]`
Multiple files contain Spanish-language comments, error messages, and UI strings. All production code, comments, error messages, and logs must be in English.
Affected files: `src/domain/models/Candidate.ts`, `src/application/services/candidateService.ts`, `src/application/services/fileUploadService.ts`, `src/index.ts`.

### 6. Hardcoded database URL in `prisma/schema.prisma` `[NEEDS REFACTOR]`
The `datasource db` block contains a hardcoded connection string with credentials. It must use an environment variable:
```prisma
datasource db {
  provider = "postgresql"
...
### 7. Hardcoded CORS origin in `src/index.ts` `[NEEDS REFACTOR]`
CORS origin is hardcoded as `'http://localhost:3000'`. It must use `process.env.FRONTEND_URL || 'http://localhost:3000'`.

### 8. Duplicate route handler for `POST /candidates` `[NEEDS REFACTOR]`
`src/routes/candidateRoutes.ts` defines an inline handler for `POST /` that calls `addCandidate` (the service function) directly, while `candidateController.ts` exports `addCandidateController` that also handles `POST /candidates`. Both exist but only the route-level inline handler is actually wired. The controller should be the single handler.

### 9. Inconsistent response format `[NEEDS REFACTOR]`
`POST /candidates` via `candidateRoutes.ts` returns raw Prisma result with no envelope. `getCandidateById` in `candidateController.ts` returns plain JSON. The standard response format (see [Request/Response Patterns](#requestresponse-patterns)) is not consistently applied.

### 10. Missing CRUD endpoints `[TODO]`
Only `POST /candidates` and `GET /candidates/:id` exist. The following are missing:
- `GET /candidates` — list all candidates
- `PUT /candidates/:id` — update candidate
- `DELETE /candidates/:id` — delete candidate
- All Position, Application, Interview CRUD endpoints

### 11. No `test:coverage` npm script `[TODO]`
`package.json` only has `"test": "jest"`. Add:
```json
...
> **`[TODO]`** `src/infrastructure/logger.ts` does not exist yet. Currently, logging is done with `console.log` and `console.error` directly in model files and services. Once the infrastructure layer is created, migrate to the pattern below.

- **Use Logger Class**: Use the centralized logger from `src/infrastructure/logger.ts` — `[TODO]`
...
The project maintains an **OpenAPI 3.0 contract** at `backend/api-spec.yaml`. The packages `swagger-jsdoc` and `swagger-ui-express` are already installed.

- **Contract-first rule**: Every new endpoint must be reflected in `api-spec.yaml` before or alongside implementation — this file is the authoritative API contract.
- **Do not** define request/response shapes only in code; they must be documented in the spec.
- Swagger UI can be wired up via `swagger-ui-express` to serve the spec at a `/api-docs` route — `[TODO]` not yet mounted in `src/index.ts`.
...
> **`[TODO]`** No tests exist in the project yet. The standards below define the target testing approach. When writing the first tests, follow these conventions from the start. The `jest.config.js` currently only has `preset: 'ts-jest'` and `testEnvironment: 'node'` — add `coverageThreshold` when adding the first test suite.
>
...
- **Threshold**: 90% for branches, functions, lines, and statements — `[TODO]` not yet configured in `jest.config.js`
- **Coverage Reports**: Generate coverage reports with `npm run test:coverage` — `[TODO]` script not yet in `package.json`
- **Coverage Files**: Coverage reports output to `coverage/` directory (auto-generated by Jest)
...
The following scripts are **currently defined** in `package.json`:
...
npm run start:prod    # Build then start (npm run build && npm start)
```

The following scripts are **not yet defined** and must be added `[TODO]`:

```bash
npm run test:coverage  # jest --coverage  [TODO]
npx prisma migrate dev --name <name>  # Create and apply a migration (run directly)
npx prisma db seed                    # Seed the database (run directly)
```
...
- **All Tests Passing**: Ensure all tests pass before deployment — `[TODO]` no tests exist yet
