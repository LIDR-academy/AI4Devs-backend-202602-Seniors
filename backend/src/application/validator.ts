...
export const validateStageUpdateBody = (data: any): void => {
  if (
    !data.applicationId ||
    !Number.isInteger(data.applicationId) ||
    data.applicationId <= 0
  ) {
    throw new Error('Invalid applicationId: must be a positive integer');
  }
  if (
    !data.newInterviewStepId ||
    !Number.isInteger(data.newInterviewStepId) ||
    data.newInterviewStepId <= 0
  ) {
    throw new Error('Invalid newInterviewStepId: must be a positive integer');
  }
};
